...
repo_id = "fzisengwe/Tourism-Package-Prediction"
...
api = HfApi(token=HF_TOKEN)  # complete the code to read the token from environment variable
...
api.upload_folder(
    folder_path="tourism_project/data",   # complete the code: path to the local data folder
    repo_id=repo_id,
    repo_type=repo_type,
)
